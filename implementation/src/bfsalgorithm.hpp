...
class BFSLocalStorage {
    public:
        vertex_t dist;
        vertex_t rep_dist;
        it_t iteration;
        local_state state;
        BFSLocalStorage() : dist(std::numeric_limits<vertex_t>::max()),
            rep_dist(std::numeric_limits<vertex_t>::max()),
            iteration(0), state(ACTIVE)
            { }
};

class BFSReplicaLocalStorage {
    public:
        vertex_t dist;
        BFSReplicaLocalStorage() : dist(std::numeric_limits<vertex_t>::max()) { }
};

class BFSVertexNotification {
    public:
        vertex_t v;
        vertex_t dist;
        BFSVertexNotification() : v(std::numeric_limits<vertex_t>::max()),
                dist(std::numeric_limits<vertex_t>::max()) { }
};

using LocalStorage = BFSLocalStorage;
using ReplicaLocalStorage = BFSReplicaLocalStorage;
using VertexNotification = BFSVertexNotification;
...
class BFSAlgorithm {
    ...
    public:
        ...
        void const query(char* d, VertexStorage &v) { *(vertex_t*)d = v.local.dist; }
        void const query(char* d) { *(vertex_t*)d = 0; }
        void set_start(vertex_t start) { start_ = start; }
};

using Algorithm = BFSAlgorithm;
